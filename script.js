...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // new TypewriterEffect(); // Disabled for debugging
...
class ProjectFilterManager {
    ...
    setupFilterButtons() {
        ...
        // Ensure first button is active on load
...
// ===== STANDALONE PROJECT FILTER FUNCTIONALITY =====
// Backup implementation to ensure filtering works
...
// Debug function for project filters
window.debugProjectFilters = function() {
    console.log('=== Project Filter Debug Info ===');
    console.log('Filter buttons:', document.querySelectorAll('.filter-btn'));
    console.log('Project cards:', document.querySelectorAll('.project-card'));
    console.log('Active filter:', document.querySelector('.filter-btn.active'));
...
class TypewriterEffect {
    constructor() {
        ...
        this.typeSpeed = 60; // Very fast typing speed (milliseconds)
        this.deleteSpeed = 30; // Very fast deleting speed (milliseconds) 
        this.pauseDuration = 1200; // Shorter pause between roles (milliseconds)
    ...
    initializeElement() {
        this.element = document.querySelector('.dynamic-role');
        
        if (this.element) {
            console.log('✅ Typewriter element found:', this.element);
            ...
            console.error('❌ Typewriter element .dynamic-role not found');
            // Try again after another delay
            setTimeout(() => this.initializeElement(), 500);
        }
    }
    ...
    startTypewriter() {
        ...
        console.log(`Current role: ${currentRole}, char index: ${this.currentCharIndex}, deleting: ${this.isDeleting}`);
        ...
        if (!this.isDeleting) {
            ...
            if (this.currentCharIndex < currentRole.length) {
                ...
                setTimeout(() => this.startTypewriter(), this.typeSpeed);
            } else {
                // Finished typing, wait then start deleting
                console.log(`Finished typing "${currentRole}", waiting before deleting...`);
                setTimeout(() => {
            ...
            if (this.currentCharIndex > 0) {
                ...
                console.log(`Moving to next role: ${this.roles[this.currentRoleIndex]}`);
                setTimeout(() => this.startTypewriter(), this.typeSpeed);
            }
        }
    }
...
// Alternative simple typewriter implementation as fallback
function startSimpleTypewriter() {
    const element = document.querySelector('.dynamic-role');
    const roles = ['Full Stack Developer', 'Backend Engineer', 'Frontend Specialist', 'UX Designer', 'Creative Thinker'];
    let roleIndex = 0;
    let charIndex = 0;
    ...
    if (element) {
        console.log('Starting simple typewriter...');
        element.textContent = '';
        type();
    }
}
